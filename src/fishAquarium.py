    ...
    def start(self):
        ...
        if (self.mode == GameMode.DEBUG):
            ...
            for i in range(0, 150):
                self.fc.addFood(rd.randint(10,self.ScreenWidth - 10), rd.randint(10,self.ScreenHeight + 10))

            numTimesToSimulate = 10
            trainingData = []
            allActions = []
            for i in range(0, numTimesToSimulate):
                self.fc.addFish(rd.randint(10,self.ScreenWidth - 10), rd.randint(10,self.ScreenHeight + 10), pg.time.get_ticks())
                elapsedTime = 0
                for j in range(0, 10):
                    self.fc.addFood(rd.randint(10,self.ScreenWidth - 10), rd.randint(10,self.ScreenHeight + 10))
                ...
                for event in pg.event.get():
                    ...
                    if event.type == pg.MOUSEBUTTONDOWN:
                        if event.button == 1: #left click
                            self.Insp.currentItem = self.fc.getAquariumItem(self.mouseX, self.mouseY, pg.time.get_ticks())
