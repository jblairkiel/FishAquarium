...
class Fish:
    ...
    def checkAndAddIfIsInVision(self, acquariumObject):
        ...
        if (
            acquariumObject.y < self.lookingBounds[1] + self.lookingBounds[3]
            and acquariumObject.y > self.lookingBounds[1]
        ) and (  # Checks x coords
            acquariumObject.x > self.lookingBounds[0]
            and acquariumObject.x < self.lookingBounds[0] + self.lookingBounds[2]
        ):  # Checks y coords
    ...
    def getNearbyFish(
        ...
        for curFish in allFish:
            if self != curFish:  # Skip the fish that are "Looking"
                if (
                    curFish.y < self.lookingBounds[1] + self.lookingBounds[3]
                    and curFish.y + curFish.lenY > self.lookingBounds[1]
                ) and (  # Checks x coords
                    curFish.x + curFish.lenX > self.lookingBounds[0]
                    and curFish.x < self.lookingBounds[0] + self.lookingBounds[2]
                ):  # Checks y coords
                    # print("NearbyFish: ")
    ...
    def generateMove(self, omitMovedCloser=False):
        ...
        if self.alive:
            ...
            while (boundX1 > tempX) or (tempX > boundX2):
                dx = rd.randint(
                    (0 - self.moveSpeed) * int(self.Health / 100),
                    self.moveSpeed * int(self.Health / 100),
                )
                tempX = self.x + dx
    ...
    def getClosestFood(self, injectedX=None, injectedY=None):
        ...
        for i in range(0, len(self.foodInVision) - 1):
            distance = math.dist(
                [tempX, tempY], [self.foodInVision[i].x, self.foodInVision[i].y]
            )
    ...
    def generateObservation(self):
        ...
        isFood = 1
        isNotFood = 0
        curObservation = np.array([self.x, self.y])
        ...
        if len(self.foodInVision) != 0:
            ...
            if len(closestValues) < targetFoodSize:
                ...
                for i in range(0, len(closestValues) - 1):
                    ...
                    curObservation = np.append(
                        self.foodInVision[closestIndex[i] - 1].x, curObservation
                    )
                    curObservation = np.append(
                        self.foodInVision[closestIndex[i] - 1].y, curObservation
                    )
    ...
    def eat(self, food):
        if self.alive:
            ...
            for foo in food:
                ...
                if (
                    foo.y - foo.radius < self.hitbox[1] + self.hitbox[3]
                    and foo.y + foo.radius > self.hitbox[1]
                ) and (  # Checks x coords
                    foo.x + foo.radius > self.hitbox[0]
                    and foo.x - foo.radius < self.hitbox[0] + self.hitbox[2]
                ):  # Checks y coords
